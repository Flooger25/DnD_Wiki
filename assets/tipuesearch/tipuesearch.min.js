(function($)
{
    ...
    {
        var set=$.extend(
            {'contentLocation':'tipuesearch/tipuesearch_content.json','contextBuffer':60,'contextLength':60,'contextStart':90,'debug':false,'descriptiveWords':25,'highlightTerms':true,'liveContent':'*','liveDescription':'*','minimumLength':3,'mode':'static','newWindow':false,'show':9,'showContext':true,'showRelated':true,'showTime':true,'showTitleCount':true,'showURL':true,'wholeWords':true}
                ,options);
        return this.each(function()
        {
            ...
            if(set.mode=='live')
            {
                for(var i=0;i<tipuesearch_pages.length;i++)
                {
                    ...
                    {
                        var cont=$(set.liveContent,html).text();cont=cont.replace(/\s+/g,' ');
                        var desc=$(set.liveDescription,html).text();desc=desc.replace(/\s+/g,' ');
            ...
            function getTipueSearch(start,replace)
            {
                ...
                if(d.length>=set.minimumLength)
                {
                    ...
                    else
                    {
                        for(var i=0;i<tipuesearch_in.pages.length;i++)
                        {
                            var score=0;var s_t=tipuesearch_in.pages[i].text;var pat=new RegExp(d,'gi');
                            if(tipuesearch_in.pages[i].title.search(pat)!=-1)
                            {
                                var m_c=tipuesearch_in.pages[i].title.match(pat).length;score+=(20*m_c);
                            }
                                ...
                                if(found[i].desc)
                                {
                                    var t=found[i].desc;if(set.showContext)
                                    {
                                        ...
                                        if(s_1>set.contextStart)
                                        {
                                            ...
                                            t_1=$.trim(t_1);if(t_1.length>set.contextLength)
                                            {
                                                t='... '+t_1;
                                            }
                                        }
